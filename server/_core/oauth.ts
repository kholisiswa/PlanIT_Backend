...
import { serializeSessionCookie, getSessionCookieOptions } from "./cookies";
...
class SDKServer {
  ...
  async createSessionToken(openId: string, options: { expiresInMs?: number; name?: string } = {}) {
    ...
    const expSeconds = Math.floor((issuedAt + expiresInMs) / 1000);
  ...
  async authenticateRequest(req: Request): Promise<User> {
    const headerCookie =
      ...
      (typeof (req as any).get === "function" ? (req as any).get("cookie") : undefined);
...
export function registerOAuthRoutes(app: Express | any) {
  ...
  app.get("/api/auth/google", async (req: Request, res: Response) => {
    ...
    const state = Buffer.from(JSON.stringify({ redirectUri, nameHint })).toString("base64");
  ...
  app.get("/api/auth/google/callback", async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!sub) return res.status(400).json({ error: "Google response missing sub" });
